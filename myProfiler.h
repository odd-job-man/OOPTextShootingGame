...
struct PROFILE_SAMPLE
{
	...
	static constexpr int MINMAX_COUNT = 5;
	LONGLONG iTotalTime;			// 전체 사용시간 카운터 Time.
	LONGLONG iMin[MINMAX_COUNT];			// 최소 사용시간 카운터 Time.	(초단위로 계산하여 저장 / [0] 가장최소 [1] 다음 최소 [2]), 두번째로 작은거, 제일작은거
	LONGLONG iMax[MINMAX_COUNT];			// 최대 사용시간 카운터 Time.	(초단위로 계산하여 저장 / [0] 가장최대 [1] 다음 최대 [2]), 두번째로 큰거, 제일 큰거
...
public:
	...
	__forceinline ~PROFILE_REQUEST()
	{
		...
		for (int i = 0; i < temp.MINMAX_COUNT - 1; ++i)
		{
			if (temp.iMax[i] < elapsed_time)
			{
				temp.iMax[i + 1] = temp.iMax[i];
				temp.iMax[i] = elapsed_time;
				break;
			}
		}
		...
		for (int i = 0; i < temp.MINMAX_COUNT - 1; ++i)
		{
			if (temp.iMin[i] > elapsed_time)
			{
				temp.iMin[i] = elapsed_time;
				break;
			}
		}
	}
...
__inline void ProfileDataOutText(void)
{
	...
	for (int i = 0; i < profiler.CURRENT_PROFILE_SAMPLE_LENGTH; ++i)
	{
		...
		float average = temp.iTotalTime * kMICRO / ((float)profiler._freq.QuadPart * (temp.iCall - num));
		float min = temp.iMin[0] * kMICRO / (float)profiler._freq.QuadPart;
		float max = temp.iMax[0] * kMICRO / (float)profiler._freq.QuadPart;
